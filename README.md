...
The lint job now fails on any ESLint error, and the quality gate workflow downloads the `.qreport/*` artifacts generated by the preceding jobs and runs `pnpm dlx tsx tooling/quality-mcp/cli/qcli.ts run --gate`, failing fast when violations are detected.
...
```bash
pnpm dlx tsx tooling/quality-mcp/cli/qcli.ts run --gate --source artifacts --scope major --task-id TR-XYZ123
```
...
# Quality gate check
pnpm dlx tsx tooling/quality-mcp/cli/qcli.ts run --gate --source artifacts --scope minor --task-id local
```
