...
import { assertOptionalString, assertOptionalStringEnum, } from '@openclaw/quality-contracts/validate/tools';
import { resolve } from 'node:path';
import { assertPathContained } from '@openclaw/quality-contracts/exec/spawn';
import { parseCoverageSummary, parseLcov, computeLcovSummary, } from '../parsers/istanbul.js';
 ...
 */
export async function coverageReportTool(input) {
    ...
    throw new Error(`NOT_FOUND: No coverage data found. Looked for:\n` +
        `  - ${summaryPath}\n` +
        `  - ${lcovPath}\n` +
...
export const coverageReportToolDef = {
    ...
    execute: async (_id, params) => {
        ...
        assertOptionalStringEnum(params['format'], 'format', ['summary', 'lcov', 'auto']);
        return coverageReportTool(params);
    },
};
