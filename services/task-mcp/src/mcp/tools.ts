...
const toolDescriptions: Record<ToolName, string> = {
  'task.create': 'Create TaskRecord in initial state',
  'task.get': 'Get TaskRecord by id',
  'task.update': 'Update TaskRecord with version control',
  'task.search': 'Search TaskRecords by text/status/labels',
  'task.transition': 'Apply state transition with business validations',
  'state.get': 'Get orchestrator state for a task',
  'state.patch': 'Update orchestrator state fields',
  'state.acquire_lock': 'Acquire an execution lease for a task',
  'state.release_lock': 'Release a previously acquired lease',
  'state.append_event': 'Register an event in the orchestrator journal',
  'state.search': 'Query orchestrator events',
  'fasttrack.evaluate': 'Evaluate fast-track eligibility with hard rules and scoring',
  'fasttrack.guard_post_dev': 'Re-evaluate fast-track after DEV (guard post-dev)',
  'quality.run_tests': 'Execute automated test suite',
  'quality.coverage_report': 'Generate coverage report',
  'quality.lint': 'Execute static analysis (lint)',
  'quality.complexity': 'Calculate complexity metrics',
  'quality.enforce_gates': 'Apply quality gates (coverage, lint, tests)',
  'gh.createBranch': 'Create a local/remote branch',
  'gh.openPR': 'Open a Pull Request',
  'gh.comment': 'Post a comment on Issue/PR',
  'gh.setProjectStatus': 'Update status in GitHub Projects',
  'gh.addLabels': 'Add labels to an Issue/PR',
  'gh.readyForReview': 'Mark a Pull Request as ready for review'
};
